...
final class ProfileDataBackupService
{
    ...
    public function exportProfile(Profile $profile): array
    {
        return [
            ...
            'favoriteMeals' => array_map(static fn (FavoriteMeal $favoriteMeal): array => ['name' => $favoriteMeal->getName(), 'description' => $favoriteMeal->getDescription(), 'createdAt' => $favoriteMeal->getCreatedAt()->format(DATE_ATOM)], $profile->getFavoriteMeals()->toArray()),
    ...
    public function importProfile(Profile $profile, string $json): int
    {
        ...
        foreach (($data['favoriteMeals'] ?? []) as $row) { $row = $this->row($row); $entity = (new FavoriteMeal())->setProfile($profile)->setName($this->requiredString($row, 'name'))->setDescription($this->requiredString($row, 'description')); if (($createdAt = $this->nullableDate($row['createdAt'] ?? null, 'createdAt')) !== null) { $entity->setCreatedAt($createdAt); } $entities[] = $entity; }
        ...
        $this->entityManager->wrapInTransaction(function () use ($profile, $displayName, $height, $birthDate, $biologicalGender, $startingWeight, $targetWeight, $entities): void {
